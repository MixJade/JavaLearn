...
* 比如导出`testdb`数据库下的`table_one`、`table_two`表

```bash
mysqldump --complete-insert --skip-quote-names -uroot -p -t testdb table_one table_two > E:\data_only.sql
```

参数解释：

* `--complete-insert`：导出的`insert`语句指定列名
* `--skip-quote-names`：导出的表名、列名不加反引号
