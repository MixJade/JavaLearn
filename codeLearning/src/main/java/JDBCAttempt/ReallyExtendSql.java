...
public class ReallyExtendSql {
    public static void main(String[] args) {
        ArrayList<StudentAndSociety> societiesList = new ArrayList<>();
        try {
            ...
            while (rs01.next()) {
                StudentAndSociety sac = new StudentAndSociety(rs01.getString(1), rs01.getInt(2), rs01.getString(3));
            ...
            while (rs02.next()) {
                StudentAndSociety sac = new StudentAndSociety(rs02.getString(1), rs02.getInt(2), rs02.getString(3));
                societiesList.add(sac);
            }
            // 关闭流
            preState.close();
...
class StudentAndSociety {
    String studentName, societyName;
    boolean sex;

    StudentAndSociety(String name, int sex, String societyName) {
        this.studentName = name;
        this.sex = sex == 1;
        this.societyName = societyName;
    }

    public void getStudentAndSociety() {
        System.out.println(" 姓名：" + String.format("%-7s",studentName) + " 性别：" + (sex ? "男" : "女") + " 社团：" + societyName);
    }
}