...
public class CXKProxy {
    public static void main(String[] args) {
        ...
        PracticeSkill cxkAgent = StarAgent.getProxy(cxk);
        System.out.println(cxkAgent.getPracticeYear());
        cxkAgent.sing();*/

        //第二种代理的方法，在建立被代理对象时就加入代理,注意用多态的写法，前面用接口承接对象
        PracticeSkill cxk02 = StarAgent.getProxy(new CXKPractice("蔡徐坤", 2.5));
...
class CXKPractice implements PracticeSkill {
    String name;
    Double practiceYear;

    CXKPractice(String name, Double practiceYear) {
        this.name = name;
    ...
    public String getPracticeYear() {
        return "这位学员练习" + practiceYear + "年";
    }

    @Override
    public void sing() {
        System.out.println(name + "开始唱歌：只因你太美");
    }
