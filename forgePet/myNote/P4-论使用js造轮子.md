        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="d-grid gap-3" id="addRoleForm">
                    ...
                    <div class="form-group row">
                        <label class="col-sm-3 col-form-label">是否住院</label>
                        <div class="col-sm-6">
                            <input class="btn-check" name="inHosRad" id="inHos_1" type="radio" autocomplete="off"
                                   checked>
                            <label class="btn btn-outline-success" for="inHos_1">居家</label>
                            <input class="btn-check" name="inHosRad" id="inHos_2" type="radio" autocomplete="off">
                            <label class="btn btn-outline-danger" for="inHos_2">住院</label>
                        </div>
                    </div>
                    <div class="form-group row">
...
<nav class="navbar bg-light sticky-top">
    <div class="container-fluid">
        <form class="d-flex" role="search" onsubmit="return false">
            ...
            <button class="btn btn-outline-success" type="submit" onclick="sss.getAll(1)">Search</button>
        </form>
...
<nav>
    <ul class="pagination justify-content-center">
        <li class="page-item">
            <select class="form-select col-sm-3" id="pageSize" onchange="sss.getAll(1)">
...
<script type="text/javascript">
    ...
    function getDataRow(h) {
        ...
        const {petId, petName, petVariety, petSex, petAge, petStatus, petBed, petPhoto, clientName} = h
        // 添加复选框
        ...
        row.appendChild(sss.sexCell(petSex, "公", "母"))
        row.appendChild(sss.cell(sss.age(petAge)))
        row.appendChild(sss.cell(petStatus))
        row.appendChild(sss.sexCell(petBed, "是", "否"))
    ...
    function formToJson() {
        const group = ["petId", "petName", "petVariety", "petAge", "petStatus", "clientId"]
        const json = {
            "petPhoto": bbb.myImg,
            "petSex": document.getElementById("petSexRad_1").checked,
            "petBed": document.getElementById("inHos_2").checked
        }
        return Object.assign(sss.getVal(group), json)
    }//json数据转表单
    function jsonToForm(h) {
        const {petSex, petBed, petPhoto} = h
        sss.setVal(["petId", "petName", "petVariety", "petAge", "petStatus", "clientId", "clientName"], h)
        bbb.change(petPhoto)
        sss.setSex(petSex, "petSexRad")
        sss.setSex(!petBed, "inHos")
    }// 查询下拉框用户名
    function getOther() {
        axios.get("http://localhost:8080/client").then(resp => changeSel(resp.data))
    }// 将用户名赋予下拉框
    function changeSel(json) {
        document.getElementById("dataOp").innerHTML = '<option data-id="" value="无">'
    ...
    const dataName = document.getElementById("clientName")
    const dataId = document.getElementById("clientId")//将id赋予隐藏栏
    dataName.onchange = () => {
